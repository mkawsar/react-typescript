export const JWT_SECRET = 'OO4Dj815gJyAXYLGQyd3rgirg6mYNQYK';
...
export const sign = (payload: Record<string, any>, privateKey: string, header: Record<string, any>) => {
    ...
    const signature = btoa(
        Array.from(encodedPayload).map((item, key) => 
            String.fromCharCode(
...
export const verify = (token: string, privateKey: any): Record<string, any> => {
    const [encodedHeader, encodedPayload, signature] = token.split('.');
    const header = JSON.parse(atob(encodedHeader));
    const payload = JSON.parse(atob(encodedPayload));
    const now = new Date();
